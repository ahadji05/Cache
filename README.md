...
public:
    ...
    // Load: Execute the database query and store results
    void load() override {
        if (m_loaded) return;  // Already loaded
        
        // Simulate expensive database query
        // In real code, you would execute: m_data = database.execute(m_query);
        std::cout << "Executing query: " << m_query << std::endl;
        
        // Simulate fetching data
        m_data.push_back("Result row 1");
        m_data.push_back("Result row 2");
        m_data.push_back("Result row 3");
        
        m_loaded = true;
    }

    // Unload: Free the cached data
    void unload() override {
        if (!m_loaded) return;  // Already unloaded
        
        std::cout << "Unloading query: " << m_query << std::endl;
...
int main() {
    ...
    // Access queries through the cache
    auto* result1 = queryCache.getItem("SELECT * FROM users");
    std::cout << "First result: " << dynamic_cast<QueryResult*>(result1)->getData()[0] << "\n";
